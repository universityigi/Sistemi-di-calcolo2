...
int write_to_pipe(int fd, const void *data, size_t data_len) {
	...
	while(written_bytes < data_len){
		...
		if(ret == -1) handle_error("Cannot write to pipe");
		written_bytes += ret;
	}
...
int read_from_pipe(int fd, void *data, size_t data_len) {
	...
	while (read_bytes < data_len){
		...
		if(ret == -1 && errno == EINTR) continue;
		if(ret == -1) handle_error("Cannot read from pipe");
		read_bytes += ret;
	}
	return read_bytes;
    /**
...
void reader(int reader_id, sem_t* read_mutex) {
    ...
    ret = close(pipefd[1]);
    if(ret) handle_error("Cannot close pipe");
    /**
     * COMPLETARE QUI
     *
     * Obiettivi:
...
int main(int argc, char* argv[]) {
    ...
    ret = pipe(pipefd);
    if(ret) handle_error("Cannot create pipe");
    /**
     * COMPLETARE QUI
     *
     * Obiettivi:
