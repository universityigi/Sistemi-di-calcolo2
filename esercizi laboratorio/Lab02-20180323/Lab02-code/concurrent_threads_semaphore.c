...
void* thread_work(void *arg) {
	...
	for (i = 0; i < m; i++) {
		if (sem_wait(&sem) != 0) {
			fprintf(stderr, "Can't lock the semaphore,error %d", errno);
			return NULL;
		}
		shared_variable += v;
		if (sem_post(&sem) != 0) {
			fprintf(stderr, "Can't unlock the semaphore,error %d", errno);
			return NULL;
		}
	}
	return NULL;
...
int main(int argc, char **argv)
{
	...
	if (sem_init(&sem,0,1) !=0){
		fprintf(stderr, "Can't initlialize the semaphore,error %d", errno);
		exit(EXIT_FAILURE);
	}
	begin(&t);
	for (i = 0; i < n; i++)
